  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Projects Stack */}
        <div className="mt-10 sm:mt-16 md:mt-24 space-y-8 sm:space-y-16 md:space-y-24">
          {projects.map((project) => (
            <article
              key={project.id}
              className="portfolio-card group relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/10 bg-[#121212] p-4 sm:p-7 md:p-10 lg:p-14 transition-all duration-500 hover:border-white/25 max-w-full"
            >
              {/* Ambient Glow */}
              <div
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8 lg:gap-10 items-stretch">
                ...
                <div className="lg:col-span-7 min-h-[200px] sm:min-h-[300px] aspect-[16/10] sm:aspect-[16/9] rounded-xl sm:rounded-2xl border border-white/10 bg-[#0A0A0A] p-4 sm:p-7 flex flex-col justify-between relative overflow-hidden group-hover:border-white/20 transition-all max-w-full">
                  ...
                  <div className="my-auto z-10 py-3 sm:py-6">
                    <span className="font-mono text-[10px] sm:text-xs uppercase tracking-widest text-gray-400">
                      {project.category}
                    </span>
                    <h3 className="mt-1.5 sm:mt-2 font-display text-xl sm:text-3xl lg:text-5xl font-light text-white tracking-tight leading-tight">
                      {project.title}
                    </h3>
                  </div>

                  <div className="z-10 border-t border-white/10 pt-2.5 sm:pt-4 flex items-center justify-between">
                ...
                <div className="lg:col-span-5 flex flex-col justify-between h-full py-1 sm:py-2">
                  <div>
                    ...
                    <h4 className="mt-3 sm:mt-5 font-display text-xl sm:text-3xl lg:text-4xl font-light text-white">
                      {project.title}
                    </h4>

                    <p className="mt-2.5 sm:mt-4 text-xs sm:text-base font-light leading-relaxed text-[#AAAAAA] text-balance">
                      {project.description}
                    </p>
                  </div>

                  <div className="mt-5 sm:mt-8 border-t border-white/10 pt-4 sm:pt-6">
                    {/* Auto Wrapping Technology Tags */}
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-5 sm:mb-8 max-w-full">
                      {project.tech.map((t, idx) => (
                        <span
                          key={idx}
                          className="rounded-full border border-white/10 bg-white/5 px-2.5 sm:px-3 py-1 font-mono text-[10px] sm:text-xs uppercase text-[#AAAAAA] break-words"
                        >
                          {t}
                        </span>
                      ))}
                    ...
                    <a
                      href="#contact"
                      className="inline-flex items-center justify-center gap-3 rounded-full bg-white px-7 py-3.5 min-h-[48px] text-xs font-semibold uppercase tracking-widest text-black transition-all duration-300 hover:bg-gray-200 active:scale-[0.98] w-full sm:w-auto text-center"
                    >
