        ...
        <div className="relative mt-12 sm:mt-20 md:mt-24">
          ...
          <div className="space-y-8 sm:space-y-16 md:space-y-20">
            {process.map((step, index) => (
              <div
                key={step.id}
                className={`process-step relative flex flex-col md:flex-row items-start justify-between gap-6 md:gap-8 ${
                  index % 2 === 1 ? "md:flex-row-reverse" : ""
                }`}
              >
                {/* Step Card */}
                <div className="w-full md:w-[45%] rounded-2xl sm:rounded-3xl border border-[#ECECEC] bg-[#F9F9FB] p-5 sm:p-8 md:p-10 transition-all duration-300 hover:border-black hover:bg-white hover:shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                  <div className="flex items-center justify-between border-b border-[#ECECEC] pb-3 sm:pb-4">
                    ...
                    <span className="font-mono text-lg sm:text-xl font-light text-black">
                      {step.id}
                    </span>
                  </div>

                  <h3 className="mt-4 sm:mt-6 font-display text-xl sm:text-2xl md:text-3xl font-light text-black tracking-tight">
                    {step.title}
                  </h3>

                  <p className="mt-3 sm:mt-4 text-xs sm:text-base font-light leading-relaxed text-[#555555]">
                    {step.description}
                  </p>
                </div>
