        ...
        <div className="mt-6 sm:mt-12 space-y-2.5 sm:space-y-4">
          {faq.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="group border-b border-[#ECECEC] py-4 sm:py-6 transition-colors duration-300"
              >
                <button
                  type="button"
                  onClick={() => setActive(isOpen ? null : item.id)}
                  className="flex w-full items-center justify-between gap-3 sm:gap-4 text-left min-h-[48px] focus:outline-none"
                >
                  <span className="font-display text-base sm:text-xl md:text-2xl font-light text-black group-hover:text-black leading-snug">
                    {item.question}
                  </span>
                  <span className="flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 min-touch items-center justify-center rounded-full border border-[#ECECEC] text-sm text-black transition-all duration-300 group-hover:border-black group-hover:bg-black group-hover:text-white">
